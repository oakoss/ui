...
| -------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| Property typo in `@layer components` (`colr: red`)                         | Yes                                                                                    | **Not caught by Tailwind compiler** — CSS/PostCSS parsers pass unknown declarations through; browser ignores at runtime |
| Invalid CSS value (`color: #zzz`)                                          | Yes                                                                                    | **Not caught by Tailwind compiler** — browsers silently ignore invalid values; no build error                           |
| Unknown at-rule (`@aply` instead of `@apply`)                              | Yes — `@aply` isn't a Tailwind at-rule, so it's not in the allowlist                   | **Not caught by Tailwind compiler** — unknown at-rules pass through to the parsed AST                                   |
...
- **Tailwind's compiler catches `@apply` semantic failures** (errors on `@apply bg-typo`) but **does not** catch generic CSS-hygiene issues like property typos, invalid values, or unknown at-rules — those pass through to the parsed output and fail silently at runtime. Stylelint catches these where the compiler doesn't, but at zero-CSS-surface that value isn't triggered yet.
